...
export default class Encodings {
  ...
  static wildcardAcceptEncoding = ['gzip', 'deflate']
  static preferredEncodings = ['br', 'gzip', 'deflate']
  static reDirective = /^\s*(gzip|compress|deflate|br|identity|\*)\s*(?:;\s*q\s*=\s*(\d(?:\.\d)?))?\s*$/
  constructor(options = {}) {
  ...
  parseAcceptEncoding(acceptEncoding = '*') {
    ...
    acceptEncoding.split(',').forEach((directive) => {
      ...
      if (!match)
        return // not a supported encoding above
  ...
  getPreferredContentEncoding() {
    const {
      encodingWeights, preferredEncodings
    } = this
